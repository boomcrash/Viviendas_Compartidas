...
public class editHuesped extends javax.swing.JFrame {
    TextPrompt prueba;
  ...
  public void datos(){         
        ...
        ResultSet rs=null;
        try {
            ps=(com.mysql.jdbc.PreparedStatement) conexion.prepareStatement("select nombres,cedula,direccion,telefono from huesped where id_huesped=?");
...
if(!jTCI.getText().isEmpty()&&!jTNombre.getText().isEmpty()&&!jTDireccion.getText().isEmpty()&&!jTTelefono.getText().isEmpty()){
            ...
            try {
                ps2=(com.mysql.jdbc.PreparedStatement) conexion.prepareStatement("update huesped set cedula=?,nombres=?,direccion=?,telefono=? where id_huesped=?");            
